...
def intensify(pixel,quantity):
    ...
    bluepix = int(pixel[2])
    if (0<=redpix+quantity<=255 and 0<=greenpix+quantity<=255 and 0<=bluepix+quantity<=255):
        return (redpix+quantity, greenpix+quantity, bluepix+quantity)
    elif (redpix+quantity<=0):
        redpix = 0
    elif (greenpix+quantity<=0):
        greenpix = 0
    elif (bluepix+quantity<=0):
        bluepix = 0
    elif (redpix+quantity>=255):
        redpix = 255
    elif (greenpix+quantity>=255):
        greenpix = 255
    elif (bluepix+quantity>=255):
        bluepix = 255
    return (redpix, bluepix, greenpix)

def brighten_helper(pixel):
...
# increases the R,G,B intensity of each pixel by 10 to 'brighten' the image
def brighten(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x, y] = brighten_helper(pixels3d[x, y])


# darken: modifies image pixel array of image_surf in place
# decreases the R,G,B intensity of each pixel by 10 to 'darken' the image
