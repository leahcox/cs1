...
-- the specific "emptySet" abstraction, representing the empty set
-- with an empty list.
emptySet = mkSet nil
...
set_member v (mkSet t) = ite (member v t)
                         true
                         false
...
list_difference (h1::t1) nil = (h1::t1)
...
--takes an element h and a list b and returns a list of all pairs
...
list_powerset (h::t) = (h::t) ++ (list_powerset t)
...
setString: (Serialize a, eq a) => set a -> String
setString (mkSet nil) = ""
setString (mkSet (h::t)) = ite (eql t nil) 
                           (toString h) 
                           ((toString h) ++ "," ++ (toString t))

set_toString s = "{" ++ (setString s) ++ "}"
